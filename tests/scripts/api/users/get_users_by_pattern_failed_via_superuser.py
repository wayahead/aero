...
print("[==]: get_users_by_pattern_failed (not found) via superadmin")
...
url = 'http://127.0.0.1:3000/api/v1/users?user_pattern=unknown_pattern'
error_flag = False
try:
...
# Handle ValueError
  ...
  if error_flag:
    print('*err: get_users_by_pattern (not found) via superadmin was failed')
    exit(1)
  else:
    print('-inf: get_users_by_pattern (not found) via superadmin was successful')
    exit(0)
