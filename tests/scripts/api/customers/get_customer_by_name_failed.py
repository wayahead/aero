...
print("[==]: get_customer_by_name_failed")
...
url = 'http://127.0.0.1:3000/api/v1/customers?customer_name=unknown'
error_flag = False
try:
...
# Handle ValueError
  ...
  if error_flag:
    print('*err: get_customer_by_name_failed (not found) was failed')
    exit(1)
  else:
    print('-inf: get_customer_by_name_failed (not found) was successful')
    exit(0)
