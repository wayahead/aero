...
print("[==]: get_users_by_pattern via admin_topease")
...
url = 'http://127.0.0.1:3000/api/v1/users?user_pattern=wrong1%'
error_flag = False
try:
...
# Handle ValueError
  ...
  if error_flag:
    print('*err: get_users_by_pattern_failed via admin_topease was failed')
    exit(1)
  else:
    print('-inf: get_users_by_pattern_failed via admin_topease was successful')

print("[==]: get_users_by_pattern via admin_bewise")
...
url = 'http://127.0.0.1:3000/api/v1/users?user_pattern=test1%'
error_flag = False
try:
  response = requests.get(url, headers=headers, timeout=5)
  # print("-inf:", response.headers)
...
# Handle ValueError
  ...
  if error_flag:
    print('*err: get_users_by_pattern via admin_bewise was failed')
    exit(1)
  else:
    print('-inf: get_users_by_pattern via admin_bewise was successful')
    exit(0)
